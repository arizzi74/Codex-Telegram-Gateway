...
func (s *Store) CreateCallback(ctx context.Context, callback Callback) (string, error) {
	...
	for range 3 {
		...
		ct, err := s.pool.Exec(ctx, `INSERT INTO telegram_callbacks
            ...
            VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (token) DO NOTHING`, token, callback.Action, callback.UserID, sessionID, approvalID, payload, callback.ExpiresAt)
		if err != nil {
...
func (s *Store) consumeCallback(ctx context.Context, tx pgx.Tx, in IncomingUpdate) (AcceptResult, error) {
	var action string
	var actor int64
	...
	err := tx.QueryRow(ctx, `SELECT action, telegram_user_id, session_id, approval_id, payload, expires_at, used_at
        FROM telegram_callbacks WHERE token=$1 FOR UPDATE`, in.CallbackToken).
	...
	markUsed := func() error {
		ct, err := tx.Exec(ctx, "UPDATE telegram_callbacks SET used_at=now() WHERE token=$1 AND used_at IS NULL", in.CallbackToken)
		if err != nil {
