...
func (s *Store) BootstrapAdmin(ctx context.Context) (string, error) {
	...
	_, err = s.pool.Exec(ctx, `INSERT INTO admin_bootstrap (bootstrap_id, token_hash, expires_at, consumed_at, created_at)
        VALUES (1, $1, now() + interval '15 minutes', NULL, now())
...
func (s *Store) CheckAdminBootstrap(ctx context.Context, token string) error {
	var actual []byte
	err := s.pool.QueryRow(ctx, `SELECT token_hash FROM admin_bootstrap WHERE bootstrap_id=1 AND consumed_at IS NULL AND expires_at>now()`).Scan(&actual)
...
func (s *Store) NewAdminCeremony(ctx context.Context, purpose string, userHandle []byte, sessionData json.RawMessage) (AdminCeremony, error) {
	...
	_, err = s.pool.Exec(ctx, `INSERT INTO admin_challenges
        ...
        VALUES ($1, $2, $3, $4, now() + interval '5 minutes', $5, $6)`, id, purpose, sha256Bytes(challenge), nullableBytes(userHandle), sessionData, hashSecret(binding))
	if err != nil {
...
func (s *Store) ReadAdminCeremony(ctx context.Context, id uuid.UUID, purpose, binding string) (AdminCeremony, error) {
	var c AdminCeremony
	var bind []byte
	err := s.pool.QueryRow(ctx, `SELECT purpose, COALESCE(user_handle, ''::bytea), session_data, ceremony_binding_hash
        FROM admin_challenges WHERE challenge_id=$1 AND consumed_at IS NULL AND expires_at > now()`, id).Scan(&c.Purpose, &c.UserHandle, &c.SessionData, &bind)
	if err != nil {
...
func (s *Store) CompleteBootstrapCredential(ctx context.Context, ceremonyID uuid.UUID, binding, bootstrap string, credential AdminCredential) error {
	...
	var actual []byte
	err = tx.QueryRow(ctx, `SELECT token_hash FROM admin_bootstrap WHERE bootstrap_id=1 AND consumed_at IS NULL AND expires_at>now() FOR UPDATE`).Scan(&actual)
	...
	if _, err = tx.Exec(ctx, `UPDATE admin_bootstrap SET consumed_at=now() WHERE bootstrap_id=1 AND consumed_at IS NULL`); err != nil {
		return err
	}
...
func (s *Store) CompleteAdminLogin(ctx context.Context, ceremonyID uuid.UUID, binding string, credential AdminCredential) (string, error) {
	...
	ct, err := tx.Exec(ctx, `UPDATE admin_credentials SET credential_json=$2,sign_count=$3,last_used_at=now() WHERE credential_id=$1 AND revoked_at IS NULL`, credential.ID, credential.CredentialJSON, record.Authenticator.SignCount)
	if err != nil {
		return "", err
	}
	...
	if _, err = tx.Exec(ctx, `INSERT INTO admin_sessions(session_id,credential_id,token_hash,expires_at) VALUES($1,$2,$3,now()+interval '8 hours')`, uuid.New(), credential.ID, hashSecret(token)); err != nil {
		return "", err
	}
...
func claimAdminCeremony(ctx context.Context, tx pgx.Tx, id uuid.UUID, purpose, binding string) error {
	var got string
	var hash []byte
	err := tx.QueryRow(ctx, `SELECT purpose,ceremony_binding_hash FROM admin_challenges WHERE challenge_id=$1 AND consumed_at IS NULL AND expires_at>now() FOR UPDATE`, id).Scan(&got, &hash)
	if err != nil || got != purpose || subtle.ConstantTimeCompare(hash, hashSecret(binding)) != 1 {
		return ErrAdminCeremonyInvalid
	}
	ct, err := tx.Exec(ctx, `UPDATE admin_challenges SET consumed_at=now() WHERE challenge_id=$1 AND consumed_at IS NULL`, id)
...
func insertAdminCredential(ctx context.Context, db adminDB, credential AdminCredential) error {
	_, err := db.Exec(ctx, `INSERT INTO admin_credentials
        ...
        VALUES ($1, $2, $3, 0, '[]'::jsonb, $4)`, credential.ID, credential.UserHandle, credential.ID, credential.CredentialJSON)
	if err != nil {
...
func (s *Store) TouchAdminCredential(ctx context.Context, credential AdminCredential) error {
	...
	ct, err := s.pool.Exec(ctx, `UPDATE admin_credentials SET credential_json=$2,sign_count=$3,last_used_at=now() WHERE credential_id=$1 AND revoked_at IS NULL`, credential.ID, credential.CredentialJSON, record.Authenticator.SignCount)
	if err != nil {
...
func (s *Store) RevokeAdminCredential(ctx context.Context, id []byte) error {
	...
	defer tx.Rollback(ctx)
	rows, err := tx.Query(ctx, `SELECT credential_id FROM admin_credentials WHERE revoked_at IS NULL FOR UPDATE`)
	if err != nil {
		return err
	}
	n := 0
	...
	ct, err := tx.Exec(ctx, `UPDATE admin_credentials SET revoked_at=now() WHERE credential_id=$1 AND revoked_at IS NULL`, id)
	if err != nil {
		return err
	}
	if ct.RowsAffected() != 1 {
	...
	_, err = tx.Exec(ctx, `UPDATE admin_sessions SET revoked_at=now() WHERE credential_id=$1 AND revoked_at IS NULL`, id)
	if err != nil {
		return err
	}
	return tx.Commit(ctx)
...
func (s *Store) CreateAdminSession(ctx context.Context, credentialID []byte) (string, error) {
	...
	id := uuid.New()
	_, err = s.pool.Exec(ctx, `INSERT INTO admin_sessions(session_id,credential_id,token_hash,expires_at) VALUES($1,$2,$3,now()+interval '8 hours')`, id, credentialID, hashSecret(token))
	if err != nil {
...
func (s *Store) ValidateAdminSession(ctx context.Context, token string) (AdminCredential, error) {
	...
	err := s.pool.QueryRow(ctx, `SELECT c.credential_id,c.user_handle,c.credential_json,c.created_at,c.last_used_at,c.revoked_at
      ...
      WHERE s.token_hash=$1 AND s.expires_at>now() AND s.revoked_at IS NULL AND c.revoked_at IS NULL`, hashSecret(token)).Scan(&c.ID, &c.UserHandle, &c.CredentialJSON, &c.CreatedAt, &c.LastUsedAt, &c.RevokedAt)
	if errors.Is(err, pgx.ErrNoRows) {
	...
	_, _ = s.pool.Exec(ctx, `UPDATE admin_sessions SET last_seen_at=now() WHERE token_hash=$1`, hashSecret(token))
	return c, nil
}
func (s *Store) RevokeAdminSession(ctx context.Context, token string) error {
	_, err := s.pool.Exec(ctx, `UPDATE admin_sessions SET revoked_at=now() WHERE token_hash=$1 AND revoked_at IS NULL`, hashSecret(token))
	if err != nil {
