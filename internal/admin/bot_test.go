...
func TestBotMonitorCachesConcurrentDashboardReads(t *testing.T) {
	a := &dashboardBotAPI{
		...
		webhook:  gateway.WebhookInfo{URL: passkeyTestOrigin + "/api/v1/telegram/webhook", PendingUpdates: 3},
	}
...
func TestDashboardRequiresAuthenticationBeforeBotCheck(t *testing.T) {
	...
	s.ServeHTTP(w, httptest.NewRequest(http.MethodGet, passkeyTestOrigin+"/api/v1/admin/dashboard", nil))
