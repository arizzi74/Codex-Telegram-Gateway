...
func TestLoadGatewayDefaultsAndSecretAllowlist(t *testing.T) {
	...
	writeFile(t, configPath, `{"database_path":"state/gateway.db","webhook_secret_env":"WEBHOOK_SECRET"}`)
	...
	if cfg.DatabasePath != filepath.Join(dir, "state", "gateway.db") {
		t.Fatalf("database path not resolved relative to config: %s", cfg.DatabasePath)
	}
...
func TestLoadGatewayRejectsAdditionalUserIdentity(t *testing.T) {
	...
	writeFile(t, path, `{"database_path":"state/gateway.db","webhook_secret_env":"WH","allowed_user_ids":[7,8]}`)
...
func TestRejectUnknownConfigurationAndPublicListener(t *testing.T) {
	...
	for _, extra := range []string{`,"allowd_user_ids":[8]`, `,"listen":"0.0.0.0:8080"`, `,"public_base_url":"http://example.com"`} {
		writeFile(t, path, `{"database_path":"state/gateway.db","webhook_secret_env":"WH"`+extra+`}`)
		if _, err := LoadGateway(path); err == nil {
...
func TestGatewayRequiresPersistentSQLitePath(t *testing.T) {
	dir := t.TempDir()
	writeSecret(t, filepath.Join(dir, ".botsecrets"), "BOTNAME=b\nBOTTOKEN=x\nWLNAME=n\nWLID=7\n")
	path := filepath.Join(dir, "gateway.json")
	for _, database := range []string{``, `,"database_path":""`, `,"database_path":":memory:"`, `,"database_path":"file:state.db"`, `,"database_path":"postgres://localhost/gateway"`, `,"database_url_env":"OLD_DATABASE"`} {
		writeFile(t, path, `{"webhook_secret_env":"WH"`+database+`}`)
		if _, err := LoadGateway(path); err == nil {
			t.Fatalf("accepted unsupported database configuration: %s", database)
		}
	}
	writeFile(t, path, `{"webhook_secret_env":"WH","database_path":"/var/lib/codex-gateway/gateway.db"}`)
	cfg, err := LoadGateway(path)
	if err != nil || cfg.DatabasePath != "/var/lib/codex-gateway/gateway.db" {
...
func TestGatewayPublicOriginAllowsConfiguredDomainsAndPreservesPort(t *testing.T) {
	...
	for _, origin := range []string{
		...
		"https://gateway.example.com/tgw/admin", "https://gateway.example.com?", "https://gateway.example.com#",
