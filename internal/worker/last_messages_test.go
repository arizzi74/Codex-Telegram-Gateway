...
func TestLastMessagesIncludesTelegramAndCLIButNeverChangesTurn(t *testing.T) {
	for _, newestFirst := range []bool{false, true} {
		for _, state := range []string{"idle", "not_loaded", "running"} {
			t.Run(fmt.Sprintf("%s/newest=%t", state, newestFirst), func(t *testing.T) {
				...
				// History must retain this correlated Telegram submission
				// alongside the CLI prompt.
				...
				command.Arguments.History = &protocol.HistoryRequest{Limit: 3, Messages: true, NewestFirst: newestFirst}
				...
				if !page.Conversation || page.NewestFirst != newestFirst || len(page.Prompts) != 0 || len(page.Messages) != 3 || page.Messages[first].Text != "CLI prompt" || page.Messages[1].Text != "Telegram prompt" || page.Messages[last].Role != "assistant" || page.Messages[last].Text != "Codex answer" {
...
func TestLastMessagesPageKeepsNewestBoundedRedactedSuffix(t *testing.T) {
	...
	page := lastMessagesPage(messages, false, 50, redactor, false)
