...
The passkey-only console is served at `/admin/`. The gateway enables it with
...
| `GET /api/v1/admin/dashboard` | Returns visible sessions and statistics, workers, runtimes, bot status, and pending counts. |
